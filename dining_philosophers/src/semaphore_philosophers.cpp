...
Utensil::Utensil(void):lock(), cond(), count(1), avail(true)
{
}

Utensil::Utensil(const Utensil &other) : lock(), cond(), count(1), avail(true)
{	
}
...
// implementation guidance from this article on semaphores in C++
// https://austingwalters.com/multithreading-semaphores/
void Utensil::wait_on(void *phil_ptr)
{
	std::unique_lock<std::mutex> lck(lock);
	if (!avail || (count == 0)){
		((Philosopher *)phil_ptr)->talk();
		cond.wait(lck);
	}
	avail = false;
	count = 0;
...
void run(Philosopher &phil, Barrier &barrier)
{
	...
	while (phil.ate < kFULL) {
		...
		bool hasLeft = true;
		phil.left.wait_on((void *)&phil);
		bool hasRight = true;
		phil.right.wait_on((void *)&phil);
		if (hasLeft && hasRight) {
			phil.eat();
			phil.left.done_with();
			phil.right.done_with();
			// phil.left.release();
			// phil.right.release();
